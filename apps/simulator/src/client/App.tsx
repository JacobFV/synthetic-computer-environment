...
function StoreApp({ computer, snapshot, setSnapshot }: { computer: ComputerSnapshot; snapshot: SimulationSnapshot; setSnapshot(value: SimulationSnapshot): void }) {
  ...
  return <div className={`store-app store-${computer.spec.os}`}><aside><h2>{storeName}</h2><label><Search size={15}/><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder={`Search ${storeName}`}/></label>{sections.map((item, index) => <button onClick={() => setSection(index)} className={section === index ? 'active' : ''} key={item}>{index === 0 ? <Store/> : item === 'Updates' || item === 'Downloads' ? <Download/> : <AppWindowIcon/>}{item}</button>)}</aside><section><div className="store-hero"><small>{computer.spec.os === 'ubuntu' ? 'EDITOR’S PICKS' : 'ESSENTIALS'}</small><h1>{section === sections.length - 1 ? 'Your software is up to date.' : 'Tools for a complete virtual workspace.'}</h1><p>signed packages from {computer.spec.os === 'windows' ? 'store.seed.local' : computer.spec.os === 'ubuntu' ? 'packages.seed.local' : 'appstore.seed.local'}</p></div><h3>{query ? 'Search results' : section === 0 ? computer.spec.os === 'windows' ? 'Top free apps' : 'Apps we love' : sections[section]}</h3><div className="store-list">{candidates.map((app) => { const installed = computer.installedApps.some((item) => item.id === app.id); return <article key={app.id}><AppIcon app={app} size={54}/><div><b>{app.name}</b><span>{app.description}</span><small>{app.publisher} · {app.version}</small></div><button disabled={installed} onClick={() => install(app.id)}>{installed ? 'OPEN' : computer.spec.os === 'windows' ? 'Get' : 'GET'}</button></article>; })}</div></section></div>;
}
